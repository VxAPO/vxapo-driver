...
//! 3. `engine::channel::default_channel_mask`（Note 17）
//!
...
pub fn resolve_channel_mask(
    ...
    crate::engine::channel::default_channel_mask(channels as u32)
}
...
mod tests {
    ...
    fn mask_from_default_when_both_zero() {
        // Note 17: 掩码为 0 时生成默认值
        let expected = crate::engine::channel::default_channel_mask(2);
        assert_eq!(resolve_channel_mask(0, None, 2), expected);
    }

    #[test]
    fn mask_from_default_when_registry_zero() {
        let expected = crate::engine::channel::default_channel_mask(8);
    ...
    fn resolve_fallback_to_51_default() {
        let expected = crate::engine::channel::default_channel_mask(6);
