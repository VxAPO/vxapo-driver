...
use crate::com::abi;
use crate::instance::audio_proc_obj::ApoObject;
...
mod tests {
    ...
    fn lock_unlock_lock_cycle() {
        ...
        assert_eq!(lock_for_process(&mut obj, &config1), abi::S_OK);
        assert_eq!(obj.state.sample_rate, 48000);
        assert_eq!(obj.state.input_channel_count, 2);

        // 解锁
        assert_eq!(unlock_for_process(&mut obj), abi::S_OK);
        assert!(!obj.state.is_locked);

        // 第二次锁定（不同配置）
        let config2 = surround_config();
        assert_eq!(lock_for_process(&mut obj, &config2), abi::S_OK);
        assert_eq!(obj.state.input_channel_count, 6);
    ...
    fn full_lock_process_unlock_cycle() {
        ...
        // Lock
        let config = stereo_config();
        assert_eq!(lock_for_process(&mut obj, &config), abi::S_OK);
        assert!(obj.state.is_locked);
