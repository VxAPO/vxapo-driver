//! instance/audio_proc_obj.rs — IAudioProcessingObject 实现
//!
//! 实现 `IAudioProcessingObject` 接口：
//! - `Reset`：重置内部处理状态
//! - `GetLatency`：返回 APO 引入的延迟
//! - `GetRegistrationProperties`：返回 APO 注册属性（`com/reg_props.rs`，Note 42）
...
//! 实时处理委托 `instance/audio_proc_obj_rt.rs`（Note 60），
//! 配置管理委托 `instance/audio_proc_obj_conf.rs`（Note 9）。
...
use crate::com::abi;
use crate::com::apo_abi::{APO_REG_PROPERTIES, REFERENCE_TIME};
use crate::com::reg_props::{props_for_clsid};
...
impl ApoObject {
    ...
    pub fn get_latency(&self, sample_rate: u32) -> REFERENCE_TIME {
        if sample_rate == 0 || self.latency_samples == 0 {
            return 0;
        }
        // 采样数 → REFERENCE_TIME：
        // latency_ref_time = samples * 10_000_000 / sample_rate
        let samples = self.latency_samples as i64;
        let rate = sample_rate as i64;
        samples * 10_000_000 / rate
    }
    ...
    pub fn query_interface(
        ...
        if iid == IUnknown::IID
            || iid == super::super::com::apo_abi::IID_IAPO
        {
            ...
            self.add_ref();
            abi::S_OK
        } else if iid == super::super::com::apo_abi::IID_IAPO_RT
            || iid == super::super::com::apo_abi::IID_IAPO_CONFIG
        {
...
mod tests {
    use windows_core::Interface;
    use crate::com::apo_abi::AUDIO_FLOW_TYPE;
    ...
    fn qi_iapo() {
        ...
        let obj = ApoObject::new(CLSID_VXAPO_PRE_MIX);
        let mut ppv: *mut c_void = std::ptr::null_mut();
        let hr = obj.query_interface(&super::super::super::com::apo_abi::IID_IAPO, &mut ppv);
        assert_eq!(hr, abi::S_OK);
        assert!(!ppv.is_null());
        obj.release();
        drop(obj);
    ...
    fn qi_iapo_rt() {
        ...
        let obj = ApoObject::new(CLSID_VXAPO_PRE_MIX);
        let mut ppv: *mut c_void = std::ptr::null_mut();
        let hr = obj.query_interface(&super::super::super::com::apo_abi::IID_IAPO_RT, &mut ppv);
        assert_eq!(hr, abi::S_OK);
        obj.release();
        drop(obj);
    }
    ...
    fn qi_iapo_config() {
        ...
        let obj = ApoObject::new(CLSID_VXAPO_PRE_MIX);
        let mut ppv: *mut c_void = std::ptr::null_mut();
        let hr = obj.query_interface(&super::super::super::com::apo_abi::IID_IAPO_CONFIG, &mut ppv);
        assert_eq!(hr, abi::S_OK);
        obj.release();
        drop(obj);
    }
