...
fn read_format_for_endpoint(endpoint_key: &RegKey -> Result<Option<AudioFormat>, VxApoError> {
    ...
    // 生成属性值名：格式为 "{GUID},PID"
    // 通道掩码（扬声器配置）
    let channel_mask_name = format!("{},0", format!("{guid}"&PKEY_AudioEndpoint_PhysicalSpeakers.fmtid;
    // 设备格式（共享模式实际使用的格式）
    let device_format_name = format!("{},0", format!("{guid}"&PKEY_AudioEngine_DeviceFormat.fmtid;
    // OEM 格式（INF 指定的默认格式）
    let oem_format_name = format!("{},0", format!("{guid}"&PKEY_AudioEngine_OEMFormat.fmtid;
