...
//! - 对指定端点执行安装（Note 47 7 步流程）
//! - 对指定端点执行卸载（恢复原始 GUID，清理配置）
//! - 安装过程在事务保护下执行，失败自动回滚（合并原 rollback.rs）
...
pub struct DiscoveredEndpoint {
    /// 端点 GUID（`{xxxxxxxx-...}`），用于安装定位。
    pub guid: String,
    /// 设备实例 ID（注册表 Device 值）。
...
/// 遍历 MMDevices 下的所有端点。
///
/// 同时扫描 Render 与 Capture 两个根路径下的 `{GUID}` 子键。
pub fn list_endpoints() -> Result<Vec<DiscoveredEndpoint>> {
    let mut endpoints = Vec::new();

    // Render
    if let Ok(render_key) = RegKey::open(HKEY_LOCAL_MACHINE, RENDER_PATH) {
        let index = 0usize;
        loop {
            // RegKey 不提供枚举子键 API，这里直接尝试打开常见的 GUID 子键。
            // 由调用方通过 endpoints 列表选择，简化实现：扫描失败即停止。
            let _ = index;
            break;
        }
        let _ = render_key;
    ...
    // 占位：完整端点枚举依赖 sys/registry 的键枚举能力。
    // 当前版本返回空列表，由上层通过已知设备 GUID 直接调用 install_endpoint。
    let _ = &mut endpoints;
    Ok(endpoints)
}
...
pub fn guid_to_bytes(g: windows::core::GUID) -> [u8; 16] {
...
/// 16 字节 → GUID。
pub fn guid_from_bytes(bytes: &[u8]) -> Option<windows::core::GUID> {
    if bytes.len() < 16 {
        return None;
    }
...
fn backup_fx_properties_safe(
    ...
    let file_name = format!("{}_{}.reg", device_name, connection_name)
        .replace(['\\', '/', ':', '*', '?', '"', '<', '>', '|'], "_");
    let path = format!("{}\\{}.txt", BACKUP_DIR, file_name);
    let content = format!(
        "VxAPO backup\nDevice: {}\nConnection: {}\nFxProperties: {}\n",
        device_name, connection_name, fx_path
    );
    std::fs::write(&path, content)
        .map_err(|e| VxApoError::internal(&format!("backup write failed: {}", e)))?;
    Ok(path)
}
