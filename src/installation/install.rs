...
//! - `device/format`：格式解析
//! - `installation/reg_write`：注册表写入与权限提升（Note 31）
...
use crate::installation::rollback::{RollbackAction, Transaction};
...
pub fn uninstall_endpoint(device_guid: &str) -> Result<()> {
    ...
    let fx_handle = match reg_write::create_key(HKEY_LOCAL_MACHINE, &fx_path) {
        Ok(h) => h,
        Err(_) => {
...
fn backup_fx_properties_safe(
    ...
    crate::installation::rollback::backup_fx_properties(
        device_name,
        connection_name,
        &fx_path,
        BACKUP_DIR,
...
mod tests {
    ...
    fn test_cleanup() {
        let _ = reg_write::delete_tree(HKEY_CURRENT_USER, TEST_FX_PATH);
    }
    ...
    fn write_child_config_with_original_apo() {
        ...
        let child = match key.read_value("childGuid").unwrap() {
            crate::utils::reg_read::RegValue::Sz(s) => s,
    ...
    fn delete_other_modes_removes_non_current() {
        ...
        for slot in ApoSlot::ALL {
            ...
            let _ = reg_write::write_sz(handle, &name, &format!("test_{}", slot.index()));
        }
    ...
    fn end_to_end_write_and_cleanup() {
        ...
        let _ = reg_write::write_sz(handle, &config.install_mode.premix_slot().value_name(), "vxapo_pre");
        let _ = reg_write::write_sz(handle, &config.install_mode.postmix_slot().value_name(), "vxapo_post");
        write_default_processmode(handle).unwrap();
