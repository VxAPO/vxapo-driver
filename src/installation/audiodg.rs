...
use crate::utils::reg_read::{RegKey, RegValue};
use crate::installation::reg_write::{self, close_key, create_key, write_dword};
...
pub fn restore() -> Result<()> {
    ...
    match reg_write::delete_value(key_handle_from_regkey(&key), VALUE_NAME) {
        Ok(()) => Ok(()),
        Err(_) => Ok(()), // 值不存在不算错误
    }
}
