...
impl<T: Copy> AlignedBuffer<T> {
    ...
    pub fn new_zeroed(len: usize, align: usize) -> Self {
        ...
        let total_bytes = len.checked_mul(elem_size).expect("buffer size overflow");
...
mod tests {
    ...
    #[test]
    fn zero_length() {
        let buf = AlignedBuffer::<f32>::new(0);
        assert!(buf.is_empty());
        assert_eq!(buf.len(), 0);
    }
    ...
    #[test]
    fn alignment_f32_32() {
        let buf = AlignedBuffer::<f32>::new(256);
    ...
    #[test]
    fn alignment_f32_16() {
        let buf = AlignedBuffer::<f32>::new_zeroed(64, 16);
        let ptr = buf.as_ptr() as usize;
    ...
    #[test]
    fn zero_initialized() {
        let buf = AlignedBuffer::<f32>::new(1024);
    ...
    fn as_mut_slice_write_read() {
        let mut buf = AlignedBuffer::<f32>::new(4);
    ...
    fn large_buffer() {
        ...
        let buf = AlignedBuffer::<f32>::new(8 * 48000);
    ...
    #[test]
    fn u8_buffer() {
        let mut buf = AlignedBuffer::<u8>::new(16);
        let slice = buf.as_mut_slice();
        for i in 0..16 {
            slice[i] = i as u8;
        }
    ...
    #[test]
    fn i16_buffer() {
        let buf = AlignedBuffer::<i16>::new_zeroed(32, 32);
