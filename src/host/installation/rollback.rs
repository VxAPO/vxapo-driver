...
pub fn backup_fx_properties(
    ...
    // 导出注册表
    read::save_to_file(HKEY_LOCAL_MACHINE, fx_properties_path, &path)?;
        ...
        for action in self.actions.drain(..).rev() {
            match action {
                RollbackAction::DeleteKey(path) => {
                    let _ = read::save_to_file(
                        HKEY_LOCAL_MACHINE,
                        &path,
                        &format!("{}.rollback_backup.reg", path.replace('\\', "_")),
                    );
                    let _ = crate::sys::registry::write::delete_tree(
                        HKEY_LOCAL_MACHINE,
                        &path,
                    );
                }
                ...
                RollbackAction::RestoreValue { root, ref key, ref name, ref backup } => {
                    ...
                    let handle = match write::create_key(root, key) {
                        Ok(h) => h,
                        Err(_) => {
                            // 尝试获取父键句柄用于权限提升。
                            if let Some((parent, _)) = key.rsplit_once('\\') {
                                if let Ok(parent_key) = crate::sys::registry::read::RegKey::open(root, parent) {
                                    let _ = write::make_writable(parent_key.handle());
                                }
                            }
                            match write::create_key(root, key) {
                                Ok(h) => h,
                                Err(e) => {
                                    log::warn!(
                    ...
                    let _ = write::write_binary(handle, name, backup);
                    write::close_key(handle);
...
mod tests {
    ...
    fn rollback_restore_value() {
        ...
        let _ = crate::sys::registry::write::delete_tree(root, &test_path);

        // 写入原始值
        let handle = crate::sys::registry::write::create_key(root, &test_path).unwrap();
        crate::sys::registry::write::write_binary(handle, "TestValue", &[1u8, 2, 3]).unwrap();
        crate::sys::registry::write::close_key(handle);

        // 覆盖为新值
        let handle = crate::sys::registry::write::create_key(root, &test_path).unwrap();
        crate::sys::registry::write::write_binary(handle, "TestValue", &[10u8, 20, 30]).unwrap();
        crate::sys::registry::write::close_key(handle);
        ...
        // 验证
        let key = crate::sys::registry::read::RegKey::open(root, &test_path).unwrap();
